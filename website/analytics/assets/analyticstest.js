...
function makeApiCall() {
  ...
  outputToPage('<img src="assets/ajax-loader.gif" width="100px" height="100px" style="padding:300px;"/>');  
...
function queryCoreReportingApi(profileId) {
  ...
  gapi.client.analytics.data.ga.get({
    'ids': 'ga:' + profileId,
    'start-date': lastNDays(period),
    'end-date': lastNDays(0),
    'metrics': 'ga:pageviews',
    'dimensions': 'ga:pagePathLevel2',
    'filters':'ga:pagePathLevel1==/questions/;ga:pagePathLevel2!@new?;ga:pagePathLevel2!@qbooks;ga:pagePathLevel2=@qbo,ga:pagePathLevel2=@quickbooks online',
    'sort': '-ga:pageviews',
    'max-results': 100
...
function handleCoreReportingResults(response) {
  if (!response.code) {
    if (response.rows && response.rows.length) {
      ...
      table.push('<thead><tr>');
      table.push('<th>#</th><th>Question</th><th>Visits</th><th>Trend</th>');
      table.push('</tr></thead><tbody>');

      // Put cells in table.
      for (var i = 0, row; row = response.rows[i]; ++i) {

        table.push('<tr><td>'+(i+1)+'</td><td><a href=https://community.intuit.com/questions'+ row[0]+' target="_blank">'+row[0]+'</a></td><td>'+ row[1]+'</td><td><img src="assets/blue_up.jpeg" width="25px" height="25px" /></td></tr>');
      }
